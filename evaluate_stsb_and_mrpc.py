...
def MRPC(epoch):
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset['train'], batch_size=16, shuffle=True, collate_fn=collate_fn)

    test_loader = torch.utils.data.DataLoader(
        dataset['validation'], batch_size=16, shuffle=True, collate_fn=collate_fn)
...
class Evaluation:
    ...
    def evaluate(self):
        ...
        if self.dataset_name == 'mrpc':
            ...
            y_pred = []
            y_true = []
            for i in range(len(stsb['validation'])):
                s1 = stsb['validation'][i]['sentence1']
                s2 = stsb['validation'][i]['sentence2']
                prob = predict_paraphrase(s1, s2, tokenizer, self.model)
            ...
            eval_loader = torch.utils.data.DataLoader(
                self.tokenized_dataset['validation'], batch_size=32, shuffle=True, collate_fn=collate_fn)
            ...
            with torch.no_grad():
                ...
                labels = self.data['validation']['label']
                # print(labels)
                # print('_________________________________________________________')
                # y = preds[0].cpu().data.numpy()
                for x in range(len(preds)):
                    preds[x] = preds[x].detach().cpu().numpy()[0]
                # print(y)
...
if __name__ == "__main__":
    ...
    a = Evaluation(model, dataset_name, number_epoch=5)
    # a.fine_tune()
    ...
    if dataset_name == 'stsb':
        ...
        print(
            f'accuracy before  fine-tuning: {accuracy_before}, f1 score before fine-tuning: {f1_score_before}')
        print(
            f'accuracy after  fine-tuning: {a.evaluate()[0]}, f1 score after  fine-tuning: {a.evaluate()[1]}')
    else:
        pass
